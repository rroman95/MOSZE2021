...
void addRowsOrColumns(myTable *Table, std::string const &input) {
	...
	int toAdd = 0;
		...
		if (input.find("rows") == 4) {						//NINCS teljes körü input ellenörzés !!!
			temp = input.substr(9);
			toAdd = stoi(temp);
			Table->addRows(toAdd);
		}

		if (input.find("columns") == 4) {						//NINCS teljes körü input ellenörzés !!!
...
void deleteRowsOrColumns(myTable* Table, std::string const &input) {
	...
	if (input.find("row") == 7) {						//NINCS teljes körü input ellenörzés !!!
	...
	if (input.find("column") == 7) {						//NINCS teljes körü input ellenörzés !!!
